...
namespace PrototipoMecanica4
{
    public class Enemy : Character
    {
        ...
        public Enemy(Vector2 initPos, Vector2 size) : base(initPos, size)
        {
            ...
            Zone = Math.Max(safeZone, dangerZone);
        }
                    ...
                    case CharacterState.Advancing:
                        {
                            if (pos.X >= Human.instance.pos.X)
                            {
                                positionToReturn.X = testBody.pos.X - pos.X + Zone; //+safedistance  
                            }
                            else
                            {
                                positionToReturn.X = testBody.pos.X - pos.X - Zone; //-safedistance
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            if (World.debugMode)
            {
                ...
                //Zone
                World.spriteBatch.Draw(World.debugZones,
                  new Vector2((pos.X >= Human.instance.pos.X ? (Human.instance.pos.X - pos.X + Zone) + (pos.X - size.X / 2) : (Human.instance.pos.X - pos.X - Zone) + (pos.X + size.X / 2)), 518f),
                  null,
                  new Color(0.0f, 0.0f, 1.0f, 0.5f),
                  0.0f,
                  Vector2.Zero, //pivot
        ...
        public void LeaveCharacterState()
        {
            switch (currentState)
            {
                ...
                case CharacterState.Advancing:
                    {
                        movingTime = 0f;
                        if (Zone > Math.Min(safeZone, dangerZone))
                        {
                            Zone -= (size.X/2);
                        }
                    }
